					  ...
					  size_t ndiscrs)
{
	...
	rc = bbdd_util_jrpc_send_done(peer, obj, &error);
	if (rc != 0)
		bbdd_mon_senderr(mon, &error, "Failed to send a session show response");
							 ...
							 uint32_t, char **))
{
	...
	obj = bbdd_jrpc_new_object(id, &error);
	if (obj == NULL) {
		bbdd_mon_senderr(mon, &error, "Failed to form a session stats response");
		return;
	}
	...
	rc = bbdd_util_jrpc_send_done(peer, obj, &error);
	if (rc != 0)
		bbdd_mon_senderr(mon, &error, "Failed to send a session stats response");
...
static void bbdd_d_bfdd_close(struct bbdd_d *d)
{
	struct bbdd_bfdd_c *bfdd = d->bfdd;

	/* BFDD issues a peer_done_cb as the peer disappears. A peer also
	 * disappears when we call bbdd_bfdd_close_c(). This would cause double
	 * frees, so unmark bfdd _before_ closing it. */
	if (bfdd != NULL) {
		d->bfdd = NULL;
		bbdd_bfdd_close_c(bfdd);
	}
}

static void bbdd_d_bfdd_peer_done_cb(void *data)
{
	struct bbdd_d *d = data;
				    ...
				    char **error)
{
	...
	cbs = (struct bbdd_bfdd_cbs) {
		.data = d,
		.peer_done_cb = bbdd_d_bfdd_peer_done_cb,
